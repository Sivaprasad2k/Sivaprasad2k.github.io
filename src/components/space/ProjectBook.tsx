...
export const ProjectBook: React.FC<ProjectBookProps> = ({
  ...
  return (
    <div
      ...
      className={`absolute w-24 h-32 transition-all duration-300 cursor-pointer z-30 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#65B8FF] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B0D10] ${
    ...
    >
      {/* Book Cover Container (#17191D Dark Cover with Walnut Spine) */}
      <div className={`w-full h-full bg-[#17191D] border-2 rounded-r-md p-2.5 flex flex-col justify-between relative overflow-hidden transition-all duration-300 shadow-2xl ${
        isFocused
        ...
        <div className="pl-2 flex items-center justify-between border-t border-[#25282D] pt-1.5 font-mono text-[8px] text-[#94a3b8]">
          ...
          <span className="text-[#7EE2A8] font-bold">NOTEBOOK</span>
        </div>
      </div>

      {/* Desk Surface Contact Shadow */}
      <div className="absolute -bottom-2 left-0 right-0 h-2 bg-black/60 blur-xs pointer-events-none" />
    </div>
  );
};
